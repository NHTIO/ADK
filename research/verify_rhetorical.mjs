...
const THREADS = [
  {
    name: 'A: core-thesis thread (rhetorical regression)',
    turns: ['what is the core thesis of ADK?', "so it's not very much of a kit, is it?"],
  },
]
...
async function main() {
  ...
  const readLastAnswer = () =>
    page.evaluate(() => {
      ...
      return { paras, lastPara, rails, toolChips, sources, plannedKind, allKinds: kindMatches }
    })
  ...
  for (const thread of THREADS) {
    ...
    for (let i = 0; i < thread.turns.length; i++) {
      ...
      while (Date.now() < deadline) {
        ...
        const lastPara = (a && a.lastPara) || ''
        const hasRealAnswer = lastPara.length > 0
        // Settle when not generating, the meter has advanced this turn, and the newest answer paragraph is
        // present + STABLE across two reads (so we don't grab a half-streamed answer).
        if (!gen && meterMoved && hasRealAnswer && lastPara === prevPara) {
          settled = true
          break
        }
        prevPara = lastPara
  ...
  try {
    ...
    for (const e of wire) {
      ...
      log({
        ...
        mentionsKit: /kit/i.test(payloadStr),
        hasNudgeOrGate:
          /provide_answer|cite|search the doc|SYSTEM-DIRECTIVE|__nudge|in your own words|documentation page/i.test(
            payloadStr
          ),
