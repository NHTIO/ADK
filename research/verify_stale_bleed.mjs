// Headed real-GPU verifier for the CROSS-TURN STALE-ARTIFACT BLEED fix (Gemma 4 E2B / LiteRT-web / WebGPU).
// Runs thread C cold — "hey, how are you?" → "what day of the week is it?" → "73291 times 8457" — loading
// the model ONCE, and captures per-turn plan kind + tool chips + committed answer + a wiretap dump, so we
// can confirm C#2 commits the CALCULATE result (619,921,187), NOT the prior turn's get_current_time string.
//
// Usage: node research/verify_stale_bleed.mjs   (preview must be serving on http://localhost:4180)
...
const THREADS = [
  {
    name: 'C: standing single-turn checks (stale-artifact bleed regression)',
