...
describe('thinking-before-tool-use profile', () => {
  ...
  it('sabotage catches a ToolCall timestamped before its Thought', () => {
    ...
    expect(result.blocking[0].ruleId).toBe('thinking-before-tool-use')
  })

  it('mutation reorders the pair and clears the violation end-to-end', async () => {
    const timeline = sabotage
    const violations = evaluateOrderingProfile(timeline, thinkingBeforeToolUse).blocking
    const repaired = repairViolations(timeline, violations)
    expect(repaired.repaired[0].strategy).toBe('reorder')
    expect(evaluateOrderingProfile(repaired.timeline, thinkingBeforeToolUse).blocking).toHaveLength(
      0
    )
    ...
    await orderingGuardDispatchMiddleware({
      profiles: [thinkingBeforeToolUse],
      action: 'mutate',
    })(ctx as never, next)
    ...
    expect(evaluateOrderingProfile(effective, thinkingBeforeToolUse).blocking).toHaveLength(0)
    ...
    expect(evaluateOrderingProfile(rebuilt, thinkingBeforeToolUse).blocking).toHaveLength(0)
  })
})
