...
const valid = [entry('call', { payload: { roleTag: 'granite-3.x' } })]
const sabotage = [entry('call', { payload: { roleTag: 'assistant_tool_call' } })]

describe('role-remap-split-tool-roles profile', () => {
  it('accepts a ToolCall carrying the Granite 3.x marker', () => {
    expect(evaluateOrderingProfile(valid, roleRemapSplitToolRoles).blocking).toHaveLength(0)
  })

  it('blocks a ToolCall carrying the wrong role marker', () => {
    const result = evaluateOrderingProfile(sabotage, roleRemapSplitToolRoles)
    expect(result.blocking).toHaveLength(1)
    expect(result.blocking[0].ruleId).toBe('granite-3-x-split-tool-roles')
  })

  it('leaves the role-remap violation unrepaired and blocks dispatch', async () => {
    const result = repairViolations(
      sabotage,
      evaluateOrderingProfile(sabotage, roleRemapSplitToolRoles).blocking
    )
    ...
    await orderingGuardDispatchMiddleware({
      profiles: [roleRemapSplitToolRoles],
      action: 'mutate',
    })(ctx as never, next)
