...
import { GeminiGenerateContentAdapter } from '../../../../../src/batteries/llm/gemini_generate_content/adapter'
import {
  E_GEMINI_GENERATE_CONTENT_CONTEXT_OVERFLOW,
  E_INVALID_GEMINI_GENERATE_CONTENT_OPTIONS,
} from '../../../../../src/batteries/llm/gemini_generate_content/exceptions'
import type { DispatchContext } from '@nhtio/adk/types'
import type { DispatchExecutorHelpers } from '@nhtio/adk/dispatch_runner'
import type { GeminiGenerateContentAdapterOptions } from '../../../../../src/batteries/llm/gemini_generate_content/types'
...
const run = async (options: Record<string, unknown>, ctx = context(new Tokenizable(''))) => {
  ...
  const adapter = new GeminiGenerateContentAdapter({ model: 'test', fetch, ...options })
  await adapter.executor()(ctx, helpers())
  return { fetch, ctx }
}

describe('Gemini generateContent autoAck and contextWindow', () => {
  it('accepts null as the explicit token-accounting opt-out', () => {
    const options: GeminiGenerateContentAdapterOptions = {
      model: 'test',
      tokenEncoding: null,
    }
  ...
  it.each([
    ['default', undefined],
    ['explicit true', true],
  ])('acks a tool-call-free response (%s)', async (_label, autoAck) => {
  ...
  it('throws its typed overflow exception before fetching', async () => {
    ...
    ).rejects.toBeInstanceOf(E_GEMINI_GENERATE_CONTENT_CONTEXT_OVERFLOW)
  ...
  it('rejects a tokenEncoding without contextWindow', async () => {
    ...
    ).rejects.toBeInstanceOf(E_INVALID_GEMINI_GENERATE_CONTENT_OPTIONS)
    expect(fetch).not.toHaveBeenCalled()
  })
})
