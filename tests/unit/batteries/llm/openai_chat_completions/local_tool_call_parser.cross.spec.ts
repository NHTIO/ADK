...
// provider's `message.tool_calls` came back empty. Native tool-calling stays authoritative; this only fires
// when `tool_calls` is empty AND the option is set. Covers: (a) default OFF (unchanged), (b) opt-in recovers
// a `<call:name{…}` call and executes it, (c) native calls present → fallback NOT consulted — across the
// non-streaming and streaming paths, plus the onRawGeneration reflection.
...
// A tool call in Gemma's `<call:name{…}` surface form — a format the endpoint does NOT lift into
// `message.tool_calls`, so it arrives as plain assistant content.
const GEMMA_CALL_TEXT = '<call:echo{"text": "hi"}>'

describe('OpenAIChatCompletionsAdapter — localToolCallParser (fallback recovery)', () => {
  describe('default OFF (backward-compatible)', () => {
    it('non-streaming: a `<call:…>` in content with empty tool_calls yields NO tool call', async () => {
      const fetchFn = vi.fn(
        cassetteFetch(singleChatCompletionCassette('c', { content: GEMMA_CALL_TEXT }))
      )
      const ctx = makeCtx(new ToolRegistry([echoTool()]))
      await new OpenAIChatCompletionsAdapter({
        model: 'm',
        stream: false,
    ...
    it('streaming: same, no tool call recovered without the option', async () => {
      const fetchFn = vi.fn(
        cassetteFetch(singleStreamingCassette('c', { deltas: [GEMMA_CALL_TEXT] }))
      )
      const ctx = makeCtx(new ToolRegistry([echoTool()]))
      await new OpenAIChatCompletionsAdapter({ model: 'm', fetch: fetchFn as never }).executor()(
        ctx,
        makeHelpers()
      )
      expect(ctx._stored.toolCalls).toHaveLength(0)
  ...
  describe('opt-in: localToolCallParser recovers the call', () => {
    ...
    it('no match in content → no tool call, terminal answer stands', async () => {
      const fetchFn = vi.fn(
        cassetteFetch(singleChatCompletionCassette('c', { content: 'just a plain prose answer' }))
      )
      const ctx = makeCtx(new ToolRegistry([echoTool()]))
      await new OpenAIChatCompletionsAdapter({
        model: 'm',
  ...
  describe('native calls win (fallback not consulted)', () => {
    it('non-streaming: native tool_calls present → parser does not double-execute', async () => {
      const fetchFn = vi.fn(
        cassetteFetch(
          singleChatCompletionCassette('c', {
