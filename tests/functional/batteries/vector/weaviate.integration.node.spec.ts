import { it, describe } from 'vitest'
import { createVectorStore } from '@nhtio/adk/batteries/vector'
import { WeaviateVectorStore } from '@nhtio/adk/batteries/vector/weaviate'
import { runVectorStoreConformance, stubEncoder } from '@nhtio/adk/batteries/vector/conformance'
import type { CollectionBuilder } from '@nhtio/adk/batteries/vector'

const url = process.env.TEST_VECTOR_WEAVIATE_URL
const d = url ? describe : describe.skip

d('WeaviateVectorStore (integration)', () => {
  const makeStore = async () => {
    const vs = await createVectorStore({
      client: WeaviateVectorStore,
      options: {
        metric: 'cosine',
        encoder: stubEncoder,
        dimensions: 3,
        connection: {
          url: url,
          apiKey: process.env.TEST_VECTOR_WEAVIATE_API_KEY,
        },
      },
    })
    await vs.connect()
    await vs.schema.dropCollectionIfExists('docs')
    await vs.schema.createCollection('docs', (c: CollectionBuilder) =>
      c.vector({ dimensions: 3, metric: 'cosine' })
    )
    return vs
  }

  it('vector conformance', async () => {
    await runVectorStoreConformance('WeaviateVectorStore', makeStore)
  })
})
